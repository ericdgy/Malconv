...
class CBPConv(nn.Module):
    def __init__(
            self,
            in_layer: [nn.Conv1d, nn.Conv2d],  # 支持 Conv1d 和 Conv2d
            out_layer: [nn.Conv1d, nn.Conv2d, nn.Linear],  # 支持 Conv1d, Conv2d 和 Linear
            ln_layer: nn.LayerNorm = None,
            bn_layer: [nn.BatchNorm1d, nn.BatchNorm2d] = None,  # 支持 BatchNorm1d 和 BatchNorm2d
        ...
        # 修改层类型检查，允许 Conv1d 和 Conv2d
        if not isinstance(in_layer, (nn.Conv1d, nn.Conv2d)):
            raise Warning("Make sure in_layer is a Conv1d or Conv2d layer")
        
        # 修改 out_layer 检查，允许 Linear, Conv1d 和 Conv2d
        if not isinstance(out_layer, (nn.Linear, nn.Conv1d, nn.Conv2d)):
            raise Warning("Make sure out_layer is a Conv1d, Conv2d, or Linear layer")
    ...
    def get_features_to_reinit(self):
        ...
        elif isinstance(self.out_layer, nn.Conv1d):
        ...
        elif isinstance(self.out_layer, nn.Conv2d):
        ...
        if isinstance(self.in_layer, nn.Conv2d) and isinstance(self.out_layer, nn.Linear):
    ...
    def reinit_features(self, features_to_replace_input_indices, features_to_replace_output_indices):
        ...
        with torch.no_grad():
            ...
            """
            if isinstance(self.bn_layer, (nn.BatchNorm1d, nn.BatchNorm2d)):
